...
def get_person_id(embedding):
    ...
    for pid, pdata in people_db.items():
        ...
        score = face_recognizer.match(
            embedding, db_embedding, cv2.FaceRecognizerSF_FR_COSINE
        )
        if score > max_score:
            max_score = score
            best_match_id = pid
...
async def detect_gender_age(
    file: UploadFile = File(...), enable_deepface: bool = False
):
    ...
    if faces is not None:
        for face in faces:
            ...
            if "gender" in people_db[person_id] and "age" in people_db[person_id]:
                ...
                padding = 20
                face_img = img[
                    max(0, y - padding) : min(y + h + padding, height),
